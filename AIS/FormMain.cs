...
namespace AIS
{
    public partial class FormMain : Form
    {
        ...
        public FormMain()
        {
            ...
            r.Write($"+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------------+\r\n" +
                    $"|  Номер  |   Размер    |   Alpha  | Gamma |  eta  | lambda | rho |  с1  |  с2  |  с3  |  K  |   h   |  L |  P  |   mu   | Cреднее значение отклонения | Наименьшее значение | Среднеквадратическое |   Кол-во   |\r\n" +
                    $"| функции |  популяции  |          |       |       |        |     |      |      |      |     |       |    |     |        |     от точного решения      |      отклонения     |      отклонение      |  успехов   |\r\n" +
                    $"+---------+-------------+----------+-------+-------+--------+-----+------+------+------+-----+-------+----+-----+--------+-----------------------------+---------------------+----------------------+------------+\r\n");
            r.Close();
            fs.Close();
                    ...
                    dataGridView1.Rows[1].Cells[2].Value != null)
                {
                    if (comboBox1.SelectedIndex != -1)
                    {
                        ...
                        {          
                            ...
                            r.Write(String.Format(@"{0,3:f5} | {1,3:f5} | {2,3:f5} | {3,3:f5} | {4,3:f5} | {5,3:f5} | {6,3:f5} | {7,3:f5} | {8,3:f5} | {9,3:f4} | {10,3:f5} | {11,3:f3 | {12,3:f3} | {13,3:f3} | {14,3:f3} | {15,14:f6} | {16,17:f6} | {17,15:f6} | {18,3} | {19,2}",
